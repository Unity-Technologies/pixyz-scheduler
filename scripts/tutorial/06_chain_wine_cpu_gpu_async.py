...
def prepare_model(pc: ProgramContext, params: dict):
...
@pixyz_schedule(queue='wine')
def convert_creo_to_pxz(pc:ProgramContext, params: dict):
    import subprocess
    import os
    input = pc.get_input_file()
    output = pc.get_output_dir('export.pxz')
    result = subprocess.run(['/usr/bin/python3wine', '/opt/exportcreo.py', input, output],
                            capture_output=True,
                            text=True)
    # You can't use return code check because we have to kill the program
    if not os.path.isfile(output):
        logger.error(f"Error in convert_creo_to_pxz: {result.stderr}")
        raise RuntimeError(f"Error in convert_creo_to_pxz: {result.stderr}")
    params['filepath'] = output
    return params
...
@pixyz_schedule(wait=False, timeout=3600)
def main(pc: ProgramContext, params: dict):
    ...
    tuple = chain(pixyz_execute.s(params,
                                 pc.clone().update(entrypoint='convert_creo_to_pxz',
                                                   raw=True)).set(queue='wine'),
                 pixyz_execute.s(pc=pc.clone().update(entrypoint='prepare_model',
                                                      raw=True, compute_only=True)).set(queue='cpu'),
                 pixyz_execute.s(pc=pc.clone().update(entrypoint='screenshot',
                                                      raw=True, compute_only=True)).set(queue='gpu')).apply_async().as_tuple()
    return tuple
#    return {"please follow this rabbit": tuple[0][0]}